...
def enroll(request):
    ...
    if request.method == "POST":
        if request.COOKIES.get("enroll_step", None):
            ...
            resp.set_cookie("enroll_step", reverse('sms:enroll-firstname'))
            return resp
...
@csrf_exempt
def enroll_first_name(request):
    response = twiml.Response()

    if request.method == "POST":
        reporter = get_reporter(request)
        if reporter:
            reporter.first_name = request.POST['Body']
            reporter.save()

            response.message("Thanks {0} - pleased to meet you. "
                             "Can I have your last name? "
                             "".format(reporter.first_name))
            resp = twilio_response(response)
            resp.set_cookie("enroll_step", reverse('sms:enroll-lastname'))
            return resp
        else:
            response.redirect(reverse('sms:enroll'))
            resp = twilio_response(response)
...
@csrf_exempt
def enroll_last_name(request):
    response = twiml.Response()

    if request.method == "POST":
        reporter = get_reporter(request)
        if reporter:
            reporter.last_name = request.POST['Body']
            reporter.save()

            response.message("Much obliged - I have you registered as "
                            "{0} {1}.\n Next, can I have your Tax "
                            "ID number?".format(reporter.first_name,
                                                reporter.last_name))
            resp = twilio_response(response)
    ...
    if request.method == "POST":
        ...
        if reporter:
            ...
            response.message("Thank you for that info. I have you "
                             "enrolled as:\n {0} {1}\n {2}\n"
                             "Is that correct?".format(reporter.first_name,
                                                       reporter.last_name,
                                                       reporter.tax_id))
            resp = twilio_response(response)
            resp.set_cookie("enroll_step", reverse('sms:enroll-confirm'))
            return resp
        else:
            response.redirect(reverse('sms:enroll'))
            resp = twilio_response(response)
    ...
    if request.method == "POST":
        ...
        if reporter:
            if "Y" in request.POST['Body'].upper():
                ...
                response.message("Excellent - let's get started.")
                response.redirect(reverse("sms:start"))
                resp = twilio_response(response)
                resp.delete_cookie("enroll_step")
                return resp
            else:
                response.message("Gotcha - I'm sorry about that. "
